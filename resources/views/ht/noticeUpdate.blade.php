...
@section('content')
    ...
    <form action="/{{htname}}/subNotice" method="post">
        ...
        <div class="row">
            <label>公告标题</label>
            <input type="text" value="{{$notices->title}}" placeholder="公告标题" name="title" class="form-control" id="titles" style="margin: 0 0 2rem 0" />
        </div>
        <div class="row">
            <label>公告内容</label>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        ue.ready(function(){
            ...
            ue.execCommand('insertHtml', '{!! $notices->bodys !!}');
            {{--// 或者直接在 编辑器的 script 内写 {!! $notices->bodys !!} --}}
        });
